...
WORKDIR=$(dirname "$(dirname "$(get_abs_path "$0")")")
...
test_lab2() {
  ...
  for testcase in "$testcase_dir"/*.tig; do
    ...
    # Handle platform differences: normalize "(null)" to empty string for comparison
    sed 's/ (null)$/ /' /tmp/output.txt > /tmp/output_normalized.txt
    sed 's/ (null)$/ /' "${ref}" > /tmp/ref_normalized.txt
    
    diff -w -B /tmp/output_normalized.txt /tmp/ref_normalized.txt
    if [[ $? != 0 ]]; then
...
main() {
  ...
  if [[ ! $os_name == "Linux" && ! $os_name == "Darwin" ]]; then
    echo "Error: Please run this grading script in a Linux or macOS system"
    exit 1
  fi
